...
use scylla_cql::frame::{
    compress_append, old_compress_append, request::query, Compression, SerializedRequest,
};
...
fn compress_append_bench(c: &mut Criterion) {
    ...
    for query in queries {
        ...
        group.bench_with_input(BenchmarkId::new("Old", query_size), &query, |b, query| {
            b.iter_batched(
                || data.clone(),
                |mut data| {
                    old_compress_append(&query[..], Compression::Lz4, &mut data).unwrap();
                },
                BatchSize::SmallInput,
            )
        });
